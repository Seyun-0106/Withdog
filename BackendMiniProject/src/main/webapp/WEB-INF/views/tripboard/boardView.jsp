...
<div class="container">
	...
	<!-- 게시글 보기 -->
	<div id="contentView">
		<a href="javascript:location.href='${url}/tripboard/boardList'" title="글목록"><img src="${url}/img/icon_arrow.png" id="arrow"/></a>
		<!-- 제목 -->
		...
		<div class="content_info">
			...
			<span>${vo.writedate}&nbsp;&nbsp;${cVO.ismodified}</span>
			<c:if test="${logid==vo.userid}">
				<a href="${url}/tripboard/boardEdit?no=${vo.no}">수정</a> /
				<a href="${url}/tripboard/boardDelete?no=${vo.no}">삭제</a>
			</c:if>
		</div>
		...
		<div id="comment">
			...
			<c:forEach var="cVO" items="${cList}">
				  <li class="list-group-item">
					  <span class="commentname">${cVO.username}</span><br/>
					  <div>
					 	 <div class="commentList">
					 	 	<label>${cVO.content}</label>
							  <c:if test="${logid==cVO.userid}">
							  <a href="#none" class="cdelete">삭제</a>
								<a href="#none" class="cmodify">수정/</a>
								<input type="hidden" name="comno" value="${cVO.comno}"/>
							  </c:if>
						  </div>
						<c:if test="${logid==cVO.userid}">
						  	<div class="commentEdit">
								  <form	method="post" action="" class="commentEditFrm">
								  	<input type="hidden" name="comno" value="${cVO.comno}"/>
								  	<textarea name="content">${cVO.content}</textarea>
				  	...
				  	<span class="commentdate">${cVO.writedate}&nbsp;&nbsp;${cVO.ismodified}</span><br/>	
				  </li>
			  </c:forEach>
			</ul>
			<c:if test="${logStatus=='Y'}">
				<form method="post" action=" ${url}/comment/insert" id="commentFrm">
